  ...
  return (
    <div className="d-flex flex-column align-items-center box-pin-border p-2">
      <InputGroup className='box-pin'>
        <Form.Control name={name} type={type} maxLength={1} className='border-0 text-center' placeholder='' isInvalid={isInvalid !== null && isInvalid}/>
      </InputGroup>
...
function InputField({ icon, placeholder, type, suffixIcon, isInvalid, validation, name }) {
  return (
    <InputGroup>
      ...
      <Form.Control className='cstm-border rounded-0' name={name} type={type} placeholder={placeholder} isInvalid={isInvalid !== null && isInvalid} />
      {suffixIcon != null ?
        <InputGroup.Text className={`bg-input-group rounded-0  ${isInvalid === true ? 'border-danger' : null}`}>
          {suffixIcon}
